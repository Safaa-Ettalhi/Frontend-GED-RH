...
import { Users, FileText, Calendar, Bell, ArrowRight, Clock, Plus, Upload } from "lucide-react"
...
export function ManagerDashboard({ user, organizationId }: ManagerDashboardProps) {
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-red-100">
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-medium text-red-600 uppercase tracking-wider">Espace Manager</h2>
                    </div>
                    <h1 className="text-4xl font-light tracking-tight text-gray-900">
                        Bonjour, <span className="font-semibold text-red-700">{user?.name || 'Manager'}</span>
                    </h1>
                    <p className="text-sm text-red-600/70">Supervision et suivi des candidatures</p>
                </div>
...
function DashboardMetric({ label, value, icon, highlight, alert, description }: DashboardMetricProps) {
    return (
        <div className={`
